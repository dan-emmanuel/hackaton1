...
let gameMode,
  ...
  player1 = {
    ...
    lifes: 0,
    setValues(objet) {
      (player1.name = objet.name), (player1.life = objet.life);
    }, //setter
    sequenceGenerator() {},
  },
  player2 = {
...
function startGame(event) {
  ...
  switch (gameMode) {
    case "solo":
      ...
      inputRemainingLife.value = player1.lifes = parseInt(
        inputLifeNumber.value
      );
      break;
    case "duel":
      inputCurrentPlayerName.value = player1.name = inputPlayerName1.value;
      ...
      for (let index = 0; index < divCases.length; index++) {
        ...
        if (element == event.target) {
          ...
          if (gameMode == "solo") {
            ...
            if (
              otherPlayer.playedSequence.length == currentLevel &&
...
function checkSequence() {
  ...
  if (gameMode == "solo") {
    ...
    modalMessage.show();
    if (
      arraysMatch(currentPlayer.playedSequence, otherPlayer.generatedSequence)
    ) {
      setTimeout(() => {
        modalMessage.hide();
      }, 1000);
      ...
      player2.sequenceGenerator();
      player1.lifes--;
      divModalMessageContent.innerHTML =
        player1.lifes == 0
          ? `you lost, last level reached : ${currentLevel}`
          : `you made at leat one mistake, you lose a life, try again`;
    ...
    let otherWon = arraysMatch(
      currentPlayer.generatedSequence,
      otherPlayer.playedSequence
    );
    let currentWon = arraysMatch(
      otherPlayer.generatedSequence,
    ...
    if (otherWon && currentWon) {
      ...
      divModalMessageContent.innerHTML = `and the winer is  ${
        otherWon ? otherPlayer.name : currentPlayer.name
      }`;
      modalMessage.show();
      return false;
    }
